...
//	设置背景色缓存
	var cookieId = getCookie('id'); //读取需要缓存的对象。
//	是否存在cookie
	if(cookieId){
		$("body").attr("id", cookieId); //是  执行cookie
	}else{
		$("body").attr("id", 'skin-blur-kiwi'); //否  执行默认值
	}
	$('body').on('click', '.template-skins > a', function(e) {
		var skin = $(this).attr('data-skin');
		$('body').attr('id', skin, 30);
		//			设置背景色cookie
		SetCookie("id", skin, 30);
